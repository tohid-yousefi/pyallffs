...
class WelchTScoreCalculator:
    def __init__(self, dataframe, drop_labels, target):
        self.dataframe = dataframe
        self.drop_labels = drop_labels
        self.target = target

    def calculate_welch_t_score(self, plot_importance=True):
        ...
        if plot_importance:
            self.plot_feature_importance(ranked_features, welch_t_scores, feature_names)

        return welch_t_scores, ranked_features

    def plot_feature_importance(self, ranked_features, welch_t_scores, feature_names):
